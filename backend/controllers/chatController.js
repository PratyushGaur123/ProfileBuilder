...
module.exports.getConversation = async function(req, res){
    try {
        ...
        if(!req.user.id || !chatWithId){
            return res.status(400).json({
                message: "Invalid Request => data not passed"
            });
        }

        const receiver = await User.findOne({_id: chatWithId});
        ...
        const conversation = await Conversation.findOne({
            participants: {
                $all: [req.user.id, chatWithId]
            }
        })
        .populate('latestMessage', 'message -_id')
        .populate('messages', 'sender receiver message -_id');
